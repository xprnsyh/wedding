        ...
        <div class="col-12 p-0">
            <div aria-label="breadcrumb" class="breadcrumb-wrapper">
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><a
                            href="{{ route('admin.index.list_template') }}">Template</a></li>
                </ol>
            </div>
        </div>
    </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <table id="datatables" class="table-scroll display dt-responsive table table-hover">
                                ...
                                <tbody>
                                    @foreach ($list_templates as $index => $template)
                                        <tr>
                                            <td>{{ ++$index }}</td>
                                            <td>
                                                <img src="{{ asset('/admin/assets/images/list_template/' . $template->logo_template) }}"
                                                    alt="Template Logo" width="50" />
                                            </td>
                                            <td>{{ $template->name }}</td>
                                            ...
                                            <td>
                                                @if($template->is_active)
                                                    <span class="iconify" id="changeActive" data-id="{{ $template->id }}" data-width="36" data-icon="fluent:checkmark-square-24-filled" style="color: green;"></span>
                                                @else
                                                    <span class="iconify" id="changeActive" data-id="{{ $template->id }}" data-width="36" data-icon="material-symbols:cancel" style="color: red;"></span>
                                                @endif
                                            </td>
                                            
                                            <td>
                                                <a href="editevents.html" class="action-icon edit edit-template"
                                                    data-toggle="modal" data-id="{{ $template->id }}"
                                                    data-target="#EditTemplateModal">
                                                    <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                                                </a>
                                                <a href="{{ route('admin.delete.list_template', ['id' => $template->id]) }}"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addTemplateModalLabel">New Template</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('admin.store.list_template') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="AccountNumber">Category</label>
                            <select class="form-control" name="category">
                                <option value="" selected disabled>Pilih Category Template</option>
                        ...
                        <div class="form-group">
                            <label for="AccountNumber">Logo Template</label>
                            <input type="file" class="form-control" name="logo_template" placeholder="type here....">
                        </div>
                    </div>
                ...
                <form action="" method="post" id="form-edit" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        <div class="form-group text-center mt-0">
                            <img id="logo_template" src="{{ asset('admin/assets/images/list_template/') }}" style="width: 100px;">
                        </div>
                        <div class="form-group mt-0">
                            <label for="TemplateName">Template Name</label>
...
@section('add-js')
    ...
    <script>
        $(document).ready(function() {
            var template_table = $("#datatables").DataTable();
            $('.edit-template').click(function() {
                $id = $(this).attr('data-id')
                $.ajax({ //create an ajax request to display.php
                    type: "GET",
                    url: "list-template/" + $id + "/edit",
                    success: function(data) {
                        console.log(data)
                        const img = "/admin/assets/images/list_template/" + data.logo_template;
                        const urledit = "/admin/list-template/" + $id + "/update";
                        $("#name").val(data.name);
                        $("#category").val(data.category);
                        $('#logo_template').attr('src', img);
                        $('#form-edit').attr('action', urledit);
                    }
                })
            });

            template_table.on('click', '#changeActive', function(e){
                let parent = e.target.closest('tr');
                $id = $(this).attr('data-id')
                console.log($id);
                $.ajax({ //create an ajax request to display.php
                    type: "GET",
                    url: "list-template/change/status/"+ $id ,
                    success: function(data) {
                        if(data['success']){
