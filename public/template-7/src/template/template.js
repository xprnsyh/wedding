...
//             console.log(schedule, event, start);
//         // Rundown
//         function rundown() {
...
(function countdown(){
    if (typeof window.EVENT != 'undefined') {
        var schedule = window.EVENT,
            event = new Date(schedule * 1000 + (7 * 3600 * 1000)).getTime(), // Tambahkan 7 jam
            start = setInterval(rundown, 1000);
        ...
        function rundown() {
            ...
            var days = Math.floor(distance / (1000 * 60 * 60 * 24)),                            // hari
                hours = Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)),      // jam
                minutes = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60)),              // menit
                seconds = Math.floor((distance % (1000 * 60)) / 1000);                          // detik
...
$(window).on('resize', function(){
    ...
    // body height
    $('body').css({'--body-height': `${$(window).height()}px`});
